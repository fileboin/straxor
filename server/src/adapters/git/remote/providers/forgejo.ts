...
export function createForgejoAdapter(config: ForgejoConfig): GitRemoteAdapter {
  ...
  const api = async <T>(path: string, opts: RequestInit = {}): Promise<T> => {
    ...
    if (!res.ok) throw new Error(`Forgejo API error: ${res.status} ${res.statusText}`);
    return res.json();
  };
  ...
  return {
    ...
    async listBranches(owner: string, repo: string): Promise<GitBranch[]> {
      ...
      return data.map((b: any) => ({ name: b.name, commitSha: b.commit?.id || "", isDefault: false, protected: b.protected }));
    },
