...
/**
 * Lago adapter — sends usage events to Lago billing API.
 * Falls back to in-memory storage for unsupported queries.
 * Docs: https://docs.getlago.com/api
 */
export function createLagoAdapter(config?: {
  baseUrl?: string;
  apiKey?: string;
}): UsageAdapter {
  const baseUrl = config?.baseUrl || process.env.LAGO_URL || "https://api.getlago.com";
  const apiKey = config?.apiKey || process.env.LAGO_API_KEY || "";
  ...
  async function lagoFetch<T>(path: string, init?: RequestInit): Promise<T | null> {
    try {
  ...
  return {
    async logEvent(raw) {
      ...
      // Push to Lago event queue
      await lagoFetch("/api/v1/events", {
        method: "POST",
        body: JSON.stringify({
          event: {
            transaction_id: event.id,
            external_customer_id: raw.userId,
            code: `${raw.provider}.${raw.model}.tokens`,
